...
function* charactersRequestSuccess(response: { data: any }): any {
  const characters = yield select(CharacterSelectors.getCharacters);

  if (!isNull(characters)) {
    const favoriteList = produce(
      characters,
      (draft: Array<Character>): Character[] => {
        return draft.filter((item) => item.isFavorite);
      },
    );

    const newList = produce(response.data, (draft: Array<Character>) => {
      if (Array.isArray(favoriteList)) {
        favoriteList.forEach((element: Character) => {
...
function* searchCharactersSuccess(response: { data: any }): any {
  const characters = yield select(CharacterSelectors.getCharacters);
...
function* searchCharacterRequest(
  api: { searchCharacters: any },
  { search }: any,
): any {
...
function* getOtherCharactersRequest(
  api: { getOtherCharacters: any },
  { payload }: any,
) {
  loadingViewRef.current?.show();
